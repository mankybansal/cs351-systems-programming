...
void eval(char *cmdline) {
    ...
    if (!builtin_cmd(argv)) {
        ...
        if ((pid = fork()) == 0) {
            ...
            // EXECUTE PROGRAM (MYSPIN,MYSPLIT,MYSTOP)
            execvp(argv[0], argv);

            // THIS IS EXECUTE ONLY IF EXECVP RETURNS 0
...
void do_bgfg(char **argv) {
    ...
    **********************/
    ...
    // CHECK IF PID/JID ARGUMENT GIVEN
    if (!argv[1]) {
        printf("%s command requires PID or %%jobid argument\n", argv[0]);
        return;
    }

    // CHECK IF ARGUMENT IS NUMBER OR % FOR PID/JID
    if (isdigit(argv[1][0])) {
    ...
    *****************************/
    ...
    // SET JOB STATE
    if (!strcmp("bg", argv[0])) {
        job->state = BG; //SET STATE AS BACKGROUND
        printf("[%d] (%d) %s", job->jid, job->pid, job->cmdline);
    } else if (!strcmp("fg", argv[0])) {
        job->state = FG; //SET STATE AS FOREGROUND
        waitfg(job->pid);  // WAIT IN FOREGROUND
    }
}
